...
# Opening external link with default browser
def patdataopen():
...
# Helpline number search functiom
def help_num():
    global jsonData
    jsonData = {
...
# Menu presented to the user at launch
def user_menu():
...
# Detecting OS and clearing the terminal/command prompt
def clear_screen( ):
    if os.name == "nt":
        os.system('cls')
...
# Recursive function to run the program again
def reruns():
